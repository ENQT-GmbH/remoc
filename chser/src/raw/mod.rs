...
pub fn channel() -> (Sender, Receiver) {
    ...
    let sender = Sender { sender_rx, receiver_tx: Some(receiver_tx), interlock: interlock.clone() };
    let receiver = Receiver { sender_tx: Some(sender_tx), receiver_rx, interlock };
    (sender, receiver)
}
